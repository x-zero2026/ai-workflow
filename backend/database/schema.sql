-- AI Workflow Center Database Schema
...
CREATE TABLE IF NOT EXISTS workflows (
    ...
    -- RAG support (for future AI assistant)
...
-- Option 1: HNSW (better performance, requires pgvector 0.5.0+)
-- Uncomment if your pgvector version supports HNSW:
CREATE INDEX IF NOT EXISTS idx_workflows_embedding ON workflows 
USING hnsw (embedding vector_cosine_ops) 
WITH (m = 16, ef_construction = 64);

-- Option 2: IVFFlat (compatible with older pgvector versions)
-- Uncomment if you want to use IVFFlat (works with 1536 dimensions):
-- CREATE INDEX IF NOT EXISTS idx_workflows_embedding ON workflows 
-- USING ivfflat (embedding vector_cosine_ops) 
-- WITH (lists = 100);

-- Note: Create the index AFTER you have inserted some workflow data (at least 100 rows recommended)
-- You can create it manually later with:
-- For HNSW: CREATE INDEX idx_workflows_embedding ON workflows USING hnsw (embedding vector_cosine_ops);
-- For IVFFlat: CREATE INDEX idx_workflows_embedding ON workflows USING ivfflat (embedding vector_cosine_ops) WITH (lists = 100);
...
COMMENT ON COLUMN workflows.embedding IS '工作流向量表示（用于RAG搜索）';
